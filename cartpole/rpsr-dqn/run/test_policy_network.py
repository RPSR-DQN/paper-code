...
from rpsp.envs.load_environments import load_environment, create_continuous_classic_env
...
def load_rpsp_policy(args, model_exp, **kwargs):
    ...
    tic = time()
    model, filter = rpsp_filter(args, data=[X_obs, X_act], x_dim=args.x_dim)
   ...
   # pi_react = get_policy[args.pi_exp](x_dim=state_dim, output_dim=args.a_dim,
    ...
    #tic = time()
    #PiUpdater = policy_updater[args.vr][args.method](pi, **kwargs)#改
    print ('took ', time() - tic)
    return filter, pp
...
def run_policy_continuous(args, flname):
    ...
    env_exp=create_continuous_classic_env(args,"CartPole-v0",'gym.env.classic_control.cartpole:CartPoleEnv',[0,2],[0,2,1,3])

...
class RFFPSR_RNN(rnn_filter.BaseRNNFilter):        
    ...
    def _process_traj(self, traj_obs, traj_act):
        ...
        if traj_obs.shape[0] <= self._fut + 3:
            ...
            return self._process_obs(data.obs), \
                ...
                data.fut_obs
    def _process_oneact(self,act):
        data=psr_base.extract_timewins_one([act])
        return self.self._process_act(data.one)
    def _process_oneobs(self,obs):
        data=psr_base.extract_timewins_one([obs])
        return self.self._process_obs(data.one)                      
    ...
    def _init_params(self, traj_obs, traj_act):
        ...
        self._t_W_o_feato =theano.shared(name='W_o_feato', value=psr.W_o_feato.astype(theano.config.floatX)) 
        self._t_W_rff2obs =theano.shared(name='W_rff2obs',value=psr.W_rff2obs.astype(theano.config.floatX))
        self._t_W_a_feata =theano.shared(name='W_a_feata', value=psr.W_a_feata.astype(theano.config.floatX))
    ...
    def yudanning(self,t_obs):
        t_ofeat=T.dot(t_obs,self._t_W_o_feato) 
        t_out=T.dot(t_ofeat,self._t_W_rff2obs)
        return t_out
    def ceshi_update_state(self, t_state, t_obs, t_act):
        t_ofeat = T.dot(t_obs,self._t_W_o_feato) 
        t_afeat = T.dot(t_act,self._t_W_a_feata) 

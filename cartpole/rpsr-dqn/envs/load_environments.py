...
def create_gym_env_exp(args,env_name):
    env = gym.make(env_name)
    env.seed(int(args.seed) + args.trial)
    env = GymEnvironment(env, discrete=True, rng=args.rng)
    return env
...
def create_env_Mcartpole(args):
    env = create_gym_mujoco_env(args, 'InvertedPendulum-v2', 'gym.envs.mujoco:InvertedPendulum', [0, 1])
    return env
...
def create_env_doublependulum(args):
    env = create_gym_mujoco_env(args, 'InvertedDoublePendulum-v2', 'gym.envs.mujoco:InvertedDoublePendulum', [0, 1, 2],
                                [0, 1, 2, 5, 6, 7])
    return env
